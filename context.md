...
- **Google Translate & Sticky Layout fixes**: Implemented the client-side Google Translate component. To prevent breaking CSS layout boundaries and fixing the sticky header behavior, removed external translation container wrappers in `layout.tsx` and applied the `notranslate` class directly to the root element tags of the `<Header />` and `<Footer />` components. Added the Blogs page link to the Footer under the Customer Support section with full translation keys across all 5 languages.
- **Privacy Policy Page Updates**: Overwrote the Privacy Policy page layout inside [page.tsx](<file:///home/mazahir/projects/work/sartaj_foods/Sartaj-website/src/app/(dashboard)/privacy/page.tsx>) with a premium static data protection policy, incorporating the old site's requirements alongside standard modern clauses, removing the previous CMS API call dependency completely.
- **Terms & Conditions Page Refactoring**: Overwrote the Terms and Conditions page layout inside [page.tsx](<file:///home/mazahir/projects/work/sartaj_foods/Sartaj-website/src/app/(dashboard)/terms/page.tsx>) with a premium comment-free static layout, incorporating the old site's terms and conditions, Specified Commercial Transactions Law details, and liquor license managers info across all 5 languages, completely removing the CMS API call dependency.
- **Contact Us Page Updates**: Refactored [page.tsx](<file:///home/mazahir/projects/work/sartaj_foods/Sartaj-website/src/app/(dashboard)/contact-us/page.tsx>) to directly render the `<StoreLocation />` component (containing the interactive map, contact details, and phone/hours information), removing the CMS API call dependency.
- **Raksha Bandhan Animated Festive Banner**: Created [RakshaBandhanBanner.tsx](file:///home/mazahir/projects/work/sartaj_foods/Sartaj-website/src/components/home/RakshaBandhanBanner.tsx) with a beautiful CSS-animated SVG vector illustrating a sister's hand tying a designer Rakhi onto her brother's wrist. The banner features localized headlines and copy for all 5 active website languages.
- **Rakhi Designer Products JSON Catalog**: Created and translated the 7 designer Rakhi products data catalog inside `rakhi-products.json` (available on both website and admin panel), featuring detailed product specifications and 10+ translated keywords for each language per product.
- **Admin Panel Autofill & Keywords Scrolling UI**: Built a standalone `ProductAutofillDropdown` component in the admin panel (restricted to the `?dev=true` query parameter) to allow one-click metadata autofill of product creation forms. Wrapped the keywords list in a scrollable `max-h-[130px]` panel to prevent form stretching.
- **Client-side Guest Mode Support**: Updated the dashboard [layout.tsx](<file:///home/mazahir/projects/work/sartaj_foods/Sartaj-website/src/app/(dashboard)/layout.tsx>) to conditionally bypass the authentication redirect and loading overlay for public paths (Home `/`, Catalog `/products`, Privacy Policy `/privacy`, Terms `/terms`, About Us `/about-us`, Contact Us `/contact-us`, and FAQ `/faq`), allowing guests to browse the catalog anonymously. Additionally, conditionally hides the `<ProductReviewForm />` component inside [ProductReviews.tsx](file:///home/mazahir/projects/work/sartaj_foods/Sartaj-website/src/components/product/ProductReviews.tsx), hides the shopping cart link icon entirely from [HeaderActions.tsx](file:///home/mazahir/projects/work/sartaj_foods/Sartaj-website/src/components/layouts/HeaderActions.tsx), hides the wishlist heart overlay on [ProductCard.tsx](file:///home/mazahir/projects/work/sartaj_foods/Sartaj-website/src/components/common/ProductCard.tsx), and hides the cart and orders links in [Footer.tsx](file:///home/mazahir/projects/work/sartaj_foods/Sartaj-website/src/components/layouts/Footer.tsx) if the guest is not logged in (`!isAuthenticated`).
- **Public Banners, Categories, Manufacturers, and Product Reviews Routes**: Removed the `authenticateCustomer` auth middleware guard from backend routes for banners, categories, manufacturers, and product reviews to make these catalog/discovery endpoints public and accessible for anonymous/guest sessions.
